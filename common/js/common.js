...
$(document).on('click', '.accordion-header', function (e) {
   //e.stopPropagation();
    $(this).closest_child('.accordion-header-toggle').click();
});

$(document).on('click', '.accordion-header-toggle', function (e) {

    e.stopPropagation();

    var currentToggle = $(this);

    var currentItem = currentToggle.closest('.accordion-item');

    var accordion = currentItem.parent();



    if(currentItem.hasClass('accordion-active')){

        currentItem.removeClass('accordion-active');
        currentItem.closest_child('.accordion-content').animate({height: '0px'},300,
            function () {
        ...
        var currentUi = currentItem.closest_child('.ui-element');

        var cnt = currentItem.closest_child('.accordion-content');

        cnt.removeClass('hidden');
        cnt.animate({height: currentUi[0].style.height},500,
            function () {
        ...
        accordion.children('.accordion-active').each(function () {
            ...
            el.closest_child('.accordion-content').animate({height: '0px'},500,
                function () {
        ...
        currentItem.addClass('accordion-active');
...
function CloseTreeMenu(){
    ...
    $('.ui-element.EskWidgetMenu').each(function () {
        ...
        $(this).on($(this).attr('openevent'), function (e) {
        ...
        $(this).on('mouseleave', function (e) {
           ...
           // $(this).children('.EskWidgetContainer').addClass('hidden')
          ...
          /*  if($(this).parents('.ui-element.EskWidgetMenu').length == 0){
            ...
            //if($(this).parent().hasClass('sectionFixedWrapper'))
            if($(this).closest('.EskWidgetContainer').length = 0)
...
$('i[widgettype=NOTE]').mouseenter(function (e) {


    var elNote = $(this);

    if((elNote.offset().top - $(window).scrollTop()) < 230){
    ...
    elNote.parents().not('[sectionfixed="true"]').css('z-index', '99999999');
    elNote.tooltip('show');
...
$('i[widgettype=NOTE]').parent().mouseleave(function (e) {
    ...
    $(this).find('i').first().tooltip('hide').tooltip('destroy');
    $(this).css('z-index', '');
   ...
   var container =  bl.parent();
    ...
    container.find('.ui-element').filter(function () {
        ...
        && (ui.position().left > bl.position().left && ui.position().left <  (bl.position().left +  bl.width())
         ...
         var openevent = $(this).attr('openevent');

        if(openevent == undefined || openevent == 'mouseenter' ){
        $(this).mouseenter().addClass('hovering');
            $(this).find('[widgettype="ICON"], .LsConstructorElement').not('.EskWidgetMenu').first().mouseenter().addClass('hovering');
            $(this).children('.LsConstructorElement.EskWidgetMenu').addClass('hovering');
...
$(document).on('click', '.ui-element[linktp="BLOCKLINK"]', (function (e) {
    var bl = $(this).closest('.ui-element');
   var container =  bl.parent();
...
$(document).on('mouseleave', '.ui-element[linktp="BLOCKLINK"]', (function (e) {
    ...
    container.find('.hovering').each(function () {
        ...
        $(this).removeClass('hovering');
         if($(e.relatedTarget).closest('.EskWidgetMenu').length == 0)
        $(this).mouseleave();
    });
...
$('.ui-element[eskEvent]').each(function () {
    ...
    events.forEach(function (obj) {
        ...
        function dinamicsNext(state, panel) {
            ...
            if(st.length == 0){
                st = panel.children('.stateActive').first();
                st.removeClass('stateActive')
            }
